...
void clear(FILE *iAFP, FILE *dfP, FILE *aifP, FILE *nifP, FILE *nameTxt, FILE *nameBin, FILE *otherTxt, FILE *otherBin,
           FILE *sourceBin);                                                         // 21
...
void moveKeyByAcct(acctIndexNode oldNode, acctIndexNode newNode, acctIndexCell ic);  // 42.2
void moveKeyByName(nameIndexNode oldNode, nameIndexNode newNode, nameIndexCell ic);  // 42.2
...
int main() {
    ...
    aifP = fopen("indexOfAcct.dat", "rb+");         //打开帐号索引文件,必须手动建立indexOfAcct.dat文件
    nifP = fopen("indexOfName.dat", "rb+");         //打开姓名索引文件,必须手动建立indexOfName.dat文件
    ...
    while ((choice = enterChoice()) != 0) {
        switch (choice) {
            case 1:  //手动插入数据,输入ctrl+z结束输入
                ...
                printf("说明 : 必须帐号在范围 1 到 %d 之间,当输入ctrl+z时候退出插入./n", MAX);
                printf("请输入用户帐号:");
                scanf("%d", &searchNum);
                ...
                while (!feof(stdin)) {
                    ...
                    insertAddr = insertWithOutIndex(dfP, dNode);  //首先向数据文件写一条数据
                    if (IsAcctIndexOn == 1) {
            ...
            case 5:  //索引管理
                ...
                switch (adminiChoice) {
                    case 1:
                        ...
                        IsAcctIndexOn == 1 ? printf("帐号索引创建成功!/n") : printf("帐号索引已停止!/n");
            ...
            case 7:  //随机查找数据
                ...
                } else  //如果不存在索引则从头往尾找
                {
                    printf("无索引查找:/n");
                    printDataFileTitle(1);
                    searchDataAddr = searchWithOutIndexByAcct(dfP, searchNum, DH);  //从第一个数据开始查找
                    while (searchDataAddr != -1) {
                        dNode = getData(dfP, searchDataAddr);
                        displaySimpleData(dfP, searchDataAddr, dNode, 1);  //显示找到的数据
                        searchDataAddr = searchWithOutIndexByAcct(dfP, searchNum, searchDataAddr + DN);  //继续找下一条
                    }
                }
                printLine(24);
                finish = clock();
            ...
            case 8:  //清空数据文件和索引文件中数据,但初始化它们
                ...
                if (makeSure - 'y' == 0 || makeSure - 'Y' == 0) {
                    clear(iAFP, dfP, aifP, nifP, nameTxt, nameBin, otherTxt, otherBin, sourceBin);
...
int enterChoice() {
    ...
    IsAcctIndexOn == 1 ? printf("帐号索引: /t  [已启动]/n") : printf("帐号索引: [已停止]/n");
    IsBalaIndexOn == 1 ? printf("金额索引: /t  [已启动]/n") : printf("金额索引: [已停止]/n");
    IsNameIndexOn == 1 ? printf("姓名索引: /t  [已启动]/n") : printf("姓名索引: [已停止]/n");
    IsSerlIndexOn == 1 ? printf("流水索引: /t  [已启动]/n") : printf("流水索引: [已停止]/n");
    printf(
        "/n输入菜单：>>/n"
        "1: 插入数据   "
        "2: 物理读取   "
...
int doAcctIndex(FILE *aifP, acctIndexCell ic) {
    ...
    sTR = searchInTreeByAcct(aifP, ic.key);  //确定在树中查找要插入关键字ic.key要插入的位置,最终指向叶子结点
    if (sTR.flag == 1)                       //在索引中已经存在则不插入
    {
        return sTR.dataAddr;  //如果已经在数据文件中存在了这条数据,则返回原来已经存在结点的地址以供重新插入
...
void insertAcctAtFull(FILE *aifP, int oldAddr, acctIndexCell ic) {
    ...
    if (oldIndex.isMostRight == 1) {
        newAddr = makeNewAcctIndexNode(
            aifP, 1, oldIndex.isLeaf);  //新的结点,若old节点是最右节点则它也是最右节点,他和old节点在同一层
    } else {
    ...
    if (oldIndex.parentP == 0)  //若头结点就是要分裂结点的父结点,此时需要重新生成父结点
    {
        ...
        oldIndex = setAcctIndex(aifP, oldIndex, oldAddr);  //把新的根结点赋值给parentIndex结点
    } else                                                 //父结点不是索引文件头
    {
    ...
    for (i = 1, j = 1; i <= moveN; i++, j++) {
        if (oldIndex.key[M - j] > ic.key)  //若老结点中最大值比ic.key大则移动老节点中的最大值 ,M-j是最外层
        {
        ...
        } else  //<ic.key       //若ic.key是当前最大的值则移动ic.key
        {
            ...
            j--;          //控制移动旧结点的次数
            ic.key = -1;  //装入addkey后再把addkey赋一个负值,让所有的结点都大于他,所以再不用装入它
        }
    }
    ...
    if (oldIndex.num == M / 2) {
        for (i = oldIndex.num; i >= 0; i--)  //在循环的过程中要完成插入新值的任务,
        {
            ...
            if (oldIndex.key[i - 1] > ic.key)  //如果比ic大需要往后移动,保留一次循环用于存放新单元
            {
...
int insertAcctAtEmpty(FILE *aifP, int addr, acctIndexCell ic) {
    ...
    for (i = aINode.num - 1; i >= 0; i--)  //循环,移动如有必要需要把原来结点中的关键字及其孩子向后移动
    {
        if (aINode.key[i] > ic.key)  //若ic.key比结点中的值aINode.key[i]大要往还要大,原来的节点和指针需要向后移动
        {
...
sATRNode searchInTreeByAcct(FILE *aifP, int acct) {
    ...
    {
        sNR = searchInNodeByAcct(aifP, sAddr, acct, 1);  //主要获取下一层节点的地址,aINode.childP[sNR.n]
    ...
    if (n == 1) {
        ...
        do {
            ...
            if (aINode.key[i] == acct)  //如果中间的key[i]正好就是要查找的值
                break;
            if (aINode.key[i] > acct)  //如果acct比最中间的值小,则说明acct在中间值的左面,上线应该下移
            {
                bigAcct = i;
...
int disposeAcctIndex(FILE *dfP, FILE *aifP, dataNode dNode, int insertAddr) {
    ...
    dataAddr = insertAddr;  //转换数据文件的所在地址,也就是将要新插入数据的地址,因已经插入所以-1
...
//-------------------------------------------------------------------------------- 19 以物理方式显示数据
//说明:不管索引,在数据文件f中一直往下读数据,直到文件最末尾那条数据
...
//-------------------------------------------------------------------------------- 21清除所有文件数据并初始化
//说明:清除数据文件和索引文件中所有的数据,但是保留他们的文件头
void clear(FILE *iAFP, FILE *dfP, FILE *aifP, FILE *nifP, FILE *nameTxt, FILE *nameBin, FILE *otherTxt, FILE *otherBin,
           FILE *sourceBin) {
    fclose(dfP);
    fclose(aifP);
    ...
    dfP = fopen("dataFile.dat", "rb+");  //以rb+的方式打开数据文件,文件会自动添加到指定位置而不会删除其它数据
...
//-------------------------------------------------------------------------------- 35数据录入
//说明:在数据里其中两项,当前时间和后序数据这两项由系统自己处理不要人工输入
...
void goOnInsertByAcct(FILE *dfP, int nDAddr, int oDAddr) {
    ...
    oDNode = getData(dfP, oDAddr);  //读取第一条关键字和nDNode的一样的数据结点,目的在于找到最后一条
    ...
    for (i = 1; i <= autoNum; i++)  //随机提取一条数据源
    {
        if ((MAX - 1) < 32767) {
            ...
            stochasticNum =
                1 + rand() * (rand() % (by + 1)) +
                rand() %
                    (cy + 1);  //产生数据源文件中的定位随机数stochasticNum,因为实现生成的数据文件中有,最大rand()=3276
        dNode = getFromDataSourceFile(
            sfP, stochasticNum);  //由随即产生的这个数在数据源文件中提取一个原始数据并加工为完整数据
        insertAddr = insertWithOutIndex(dfP, dNode);  //首先插入数据,然后整理索引,如果帐号索引启动的话
        if (IsAcctIndexOn == 1) {
            disposeAcctIndex(dfP, aifP, dNode, insertAddr);
        }
        ...
        if (i % 1000 == 0) {
            ...
            printf("/t/t总耗时: ");  //在演示时候可以不屏蔽这三条语句,在插入时候如果显示插入速度很慢
            timeShow(duration);
            printf("/t/t预计还剩: ");  //在演示时候可以不屏蔽这三条语句,在插入时候如果显示插入速度很慢
            timeShow(leaveTime);
            printf("/n");
            tt = finish;
        }
...
dataNode doSNTDN(sourceDataCell sCell, int seconds, int serial) {
    ...
    intToString(seconds, 10, timeString);  //将时间转换成前10位字符串,其实需要11位来装它,因为还有最后的'/0'
    intToString(serial, 5, serialString);  //将流水号转换成后6位字符串,其实需要7位来装它,因为还有最后的'/0'
...
void moveKeyByAcct(acctIndexNode oldNode, acctIndexNode newNode, acctIndexCell ic) {
    ...
    moveN = (M % 2 == 0) ? (M / 2) : (M / 2 + 1);  //新结点中移进的关键字及其孩子个数
    for (i = 1, j = 1; i <= moveN; i++, j++)  //移入新结点个数,包括ic,这里不用调用直接插入函数效率更高
    {
        if (oldNode.key[M - j] > ic.key)  //若老结点中最大值比ic.key大则移动老节点中的最大值 ,M-j是最外层
        {
...
//-------------------------------------------------------------------------------- 44写日志文件
//说明:将记录以文本格式记录于文本文件中，有待完善，一般主要用语查错之用
...
void programmeDoor() {
    ...
    t = time(NULL);                                    //获得当前时间
                                                       //数据文件
    if ((dfP = fopen("dataFile.dat", "rb+")) == NULL)  //打开数据文件,如果不存在数据文件自动创建文件
    {
        dfP = fopen("dataFile.dat", "w");
        fclose(dfP);
    }
    //账号索引文件
    if ((aifP = fopen("indexOfAcct.dat", "rb+")) == NULL)  //打开数据文件,如果不存在账号索引文件文件自动创建文件
    {
        aifP = fopen("indexOfAcct.dat", "w");
        fclose(aifP);
    }
    //姓名索引文件
    if ((nifP = fopen("indexOfName.dat", "rb+")) == NULL)  //打开数据文件,如果不存在账号索引文件文件自动创建文
    {
        nifP = fopen("indexOfName.dat", "w");
        fclose(nifP);
    }
    //时间索引文件
    if ((tifP = fopen("timeOfAcct.dat", "rb+")) == NULL)  //打开数据文件,如果不存在账号索引文件文件自动创建文件
    {
        tifP = fopen("timeOfAcct.dat", "w");
        fclose(tifP);
    }
    //账号管理文件
    if ((iAFP = fopen("indexAdministrator.dat", "rb+")) == NULL)  //如果文件不存在自动创建
    {
        iAFP = fopen("indexAdministrator.dat", "w");
        fclose(iAFP);
    }
    ...
    secToDate(t);  //显示系统时间
    printf("/n数据文件中:有效数据结点数:%-d/t总数据结点数%-d/n", dHead.dENum, dHead.dANum);
    printf("帐号索引中:有效索引结点数:%-d/t总索引结点数%-d/t树的层数:%d/n", aiHead.iENum, aiHead.iANum, aiHead.tLayer);
    printf("=============================== 索引状态===============================/n帐号索引:%s /n",
...
int indexMenu(FILE *iAFP) {
    ...
    if (menuChoice == 1) {
        printf("确认要 %s 帐号索引? 是(Y) , 否(N)  /n>>", iaNode.isAccountIndexOn == 1 ? "<停止>" : "<启动>");
        fflush(stdin);
        makeSure = getchar();
        ...
        printf("确认要 %s 金额索引? 是(Y) , 否(N)  /n>>", iaNode.isBalanceIndexOn == 1 ? "<停止>" : "<启动>");
        fflush(stdin);
        makeSure = getchar();
        if (makeSure == 'Y' || makeSure == 'y') {
            intChoice = 2;
        ...
        printf("确认要 %s 姓名索引? 是(Y) , 否(N)  /n>>", iaNode.isNameIndexOn == 1 ? "<停止>" : "<启动>");
        fflush(stdin);
        makeSure = getchar();
        if (makeSure == 'Y' || makeSure == 'y') {
            intChoice = 3;
        ...
        printf("确认要 %s 流水号索引? 是(Y) , 否(N)  /n>>", iaNode.isSerialumIndexOn == 1 ? "<停止>" : "<启动>");
        fflush(stdin);
        makeSure = getchar();
        if (makeSure == 'Y' || makeSure == 'y') {
            intChoice = 4;
...
int insertWithOutIndex(FILE *dfP, dataNode dNode) {
    ...
    dHead = getDataHead(dfP);
    dataAddr = dataCellToAddr(dHead.dANum);  //转换数据文件的所在地址,也就是将要新插入数据的地址
...
indexAdmiNode printIndexState(FILE *iAFP) {
    ...
    iaNode = getIndexAdmiNode(iAFP);
    iaNode.isAccountIndexOn == 1 ? printf("帐号索引: /t  [已启动]/n") : printf("帐号索引: [已停止]/n");
    iaNode.isBalanceIndexOn == 1 ? printf("金额索引: /t  [已启动]/n") : printf("金额索引: [已停止]/n");
    iaNode.isNameIndexOn == 1 ? printf("姓名索引: /t  [已启动]/n") : printf("姓名索引: [已停止]/n");
    iaNode.isSerialumIndexOn == 1 ? printf("流水索引: /t  [已启动]/n") : printf("流水索引: [已停止]/n");
    return iaNode;
}
...
void rebulidAcctIndex(FILE *dfP, FILE *aifP) {
    ...
    for (i = 0; i < DANum; i++) {
        dNode = getData(dfP, i * DN + DH);
        dNode = regressDataNext(dfP, i * DN + DH, dNode);  //让数据的所有next全部回复到初始的-1状态
        if (dNode.acct > 0)
...
void getManualInput(FILE *aifP, FILE *dfP, FILE *sfP) {
    ...
    printf("手工输入信息,帐号为正整数,金额必须是数值,范围从1到 %d (当输入 0 退出插入.)/n", MAX);
...
void readTxtNameWriteBin(FILE *nameTxt, FILE *nameBin) {
    ...
    fscanf(nameTxt, "%d%s", &nNode.no, nNode.name);  //从 txt 文件指针所指向的文件中读取输入，存入到默认结构体中
    while (!feof(nameTxt))                           //如果未到文件结尾
    {
        fseek(nameBin, (nNode.no - 1) * sizeof(nameNode), SEEK_SET);  //将文件的指针定位到文件的正确记录
...
void readTxtOtherWriteBin(FILE *otherTxt, FILE *otherBin) {
    ...
    fscanf(otherTxt, "%d%s", &oNode.no, oNode.other);  //从 otherTxt 文件指针所指向的文件中读取输入，存入到默认结构体中
    while (!feof(otherTxt))                            //如果未到文件结尾
    {
        fseek(otherBin, (oNode.no - 1) * sizeof(otherNode), SEEK_SET);  //将文件的指针定位到文件的正确记录
...
void composeFile(int num, FILE *nameBin, FILE *otherBin, FILE *sourceBin) {
    ...
    for (i = 1; i <= num + 1; i++) {
        ...
        fwrite(&sdC, sizeof(sourceDataCell), 1, sourceBin);  //顺序写入到数据文件data.dat中
        if (i % 1000 == 0) {
            printf("/r");
            printf("完成第 %d 条", i);
        }
...
//-------------------------------------------------------------------------------- 69时间显示
//说明：把秒数已hh:mm:ss mmm格式显示，最后的三个m表示三位毫秒
