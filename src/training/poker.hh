...
namespace poker {
    ...
    public:
        constexpr const static unsigned CARD_DATA[54] = {
                ...
                0x0B, 0x0C, 0x0D,    //方块 A - K
                0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A,
                0x1B, 0x1C, 0x1D,    //梅花 A - K
                0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A,
                0x2B, 0x2C, 0x2D,    //红桃 A - K
                0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3A,
                0x3B, 0x3C, 0x3D,    //黑桃 A - K
                0x4E, 0x4F,
        };
    ...
    public:
        ...
        // 此函数入参花色必须为 形如黑桃0x30
        void SortHandCardListByPrimary(unsigned cbCardData[], unsigned cbCardCount, const bool bDragonData[],
                                       unsigned specificColor);

        bool findStraightType(const unsigned byHandCardData[], unsigned byHandCardCount, unsigned &byReferCard,
                              unsigned countType, unsigned countMaxStraight, unsigned byPrimaryColor);
        //甩牌分析模块
    public:

        unsigned findMaxLogicValue(const unsigned *cbCardData, unsigned cbCardCount, unsigned cbPrimaryColor,
                                   unsigned cbCurrentColor);

        bool analyseCardData(const unsigned *cbCardData, unsigned cbCardCount, tagSameAnalyseResult &AnalyseResult,
                             unsigned cbPrimaryColor);

        bool compareThrowCard(const unsigned cbFirstCard[], const unsigned cbNextCard[], unsigned cbFirstCount,
                              unsigned cbNextCount, unsigned cbPrimaryColor, unsigned cbCurrentColor);

        //searchSameCard searchLineCardType 方法里的 cbCurrentColor 无须过虑 只用花色判断即可
        unsigned searchSameCard(const unsigned *cbHandCardData, unsigned cbHandCardCount, unsigned cbReferCard,
                                unsigned cbSameCardCount, tagSearchCardResult &pSearchCardResult,
                                unsigned cbPrimaryColor, unsigned cbCurrentColor);

        unsigned searchLineCardType(const unsigned *cbHandCardData, unsigned cbHandCardCount, unsigned cbReferCard,
                                    unsigned cbBlockCount, unsigned cbLineCount, tagSearchCardResult &pSearchCardResult,
                                    unsigned cbPrimaryColor, unsigned cbCurrentColor);
        ...
        bool searchOutCard(const unsigned *cbHandCardData, unsigned cbHandCardCount, const unsigned *cbTurnCardData,
                           unsigned cbTurnCardCount, tagSearchCardResult &pSearchCardResult, unsigned cbPrimaryColor,
                           unsigned cbCurrentColor);
    ...
    public:
        ...
        bool RemoveExist(std::vector<unsigned> &source_arr, unsigned source_length, std::vector<unsigned> delete_arr,
