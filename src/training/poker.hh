...
namespace poker {
    ...
    public:
        ...
        ~Poker() = default;
    public:
        constexpr const static unsigned CARD_DATA[54] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,    //方块 A - K
    ...
    public:
        ...
        // 此函数入参花色必须为 形如黑桃0x30
        void SortHandCardListByPrimary(unsigned cbCardData[], unsigned cbCardCount, const bool bDragonData[], unsigned specificColor);
        bool findStraightType(const unsigned byHandCardData[], unsigned byHandCardCount, unsigned &byReferCard, unsigned countType, unsigned countMaxStraight, unsigned byPrimaryColor);
        //甩牌分析模块
    public:

        unsigned findMaxLogicValue(const unsigned *cbCardData, unsigned cbCardCount, unsigned cbPrimaryColor, unsigned cbCurrentColor);
        bool analyseCardData(const unsigned *cbCardData, unsigned cbCardCount, tagSameAnalyseResult &AnalyseResult, unsigned cbPrimaryColor);
        bool compareThrowCard(const unsigned cbFirstCard[], const unsigned cbNextCard[], unsigned cbFirstCount, unsigned cbNextCount, unsigned cbPrimaryColor, unsigned cbCurrentColor);
        //searchSameCard searchLineCardType 方法里的 cbCurrentColor 无须过虑 只用花色判断即可
        unsigned searchSameCard(const unsigned *cbHandCardData, unsigned cbHandCardCount, unsigned cbReferCard, unsigned cbSameCardCount, tagSearchCardResult &pSearchCardResult, unsigned cbPrimaryColor, unsigned cbCurrentColor);
        unsigned searchLineCardType(const unsigned *cbHandCardData, unsigned cbHandCardCount, unsigned cbReferCard, unsigned cbBlockCount, unsigned cbLineCount, tagSearchCardResult &pSearchCardResult, unsigned cbPrimaryColor, unsigned cbCurrentColor);
        ...
        bool searchOutCard(const unsigned *cbHandCardData, unsigned cbHandCardCount, const unsigned *cbTurnCardData, unsigned cbTurnCardCount, tagSearchCardResult &pSearchCardResult, unsigned cbPrimaryColor, unsigned cbCurrentColor);
    ...
    public:
        ...
        bool RemoveExist(std::vector<unsigned> &source_arr, unsigned source_length, std::vector<unsigned> delete_arr, unsigned delete_length);
